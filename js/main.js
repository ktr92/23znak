$(document).ready(function () {
	...
	try {
		$('.mainslider__slider').each(function () {
			$(this).slick({
				...
				dots: true,
				fade: true,
				  nextArrow: $(this).parent().find('.sliderarrows__right'),
				  prevArrow: $(this).parent().find('.sliderarrows__left'),
			});
		});
	} catch (err) {
	}
	...
	try {
		$('.productslider__slider-four').each(function () {
			$(this).slick({
				infinite: false,
				slidesToShow: 4,
				slidesToScroll: 1,
	 ...
	 try {
		var showMoreButtons = document.querySelectorAll(".productcard__showMore");
		showMoreButtons.forEach(function (btn) {
		  btn.addEventListener("click", function () {
			var parent = btn.parentElement;
			var text = parent.querySelector(".productcard__desc");
